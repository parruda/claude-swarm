...
module SwarmSDK
  class ResultTest < Minitest::Test
    ...
    def test_total_cost_with_logs
      logs = [
        { type: "llm_response", usage: { total_cost: 0.01 } },
        { type: "llm_response", usage: { total_cost: 0.02 } },
        { type: "llm_response", usage: { total_cost: 0.03 } },
      ]
      result = Result.new(agent: "test", logs: logs)

      assert_in_delta(0.06, result.total_cost)
    end
    ...
    def test_total_cost_with_missing_usage
      logs = [
        ...
        { type: "llm_response" }, # No usage field
        { type: "llm_response", usage: { total_cost: 0.02 } },
      ]
      result = Result.new(agent: "test", logs: logs)
    ...
    def test_total_tokens_with_logs
      logs = [
        ...
        { type: "llm_response", usage: { total_tokens: 200 } },
        { type: "llm_response", usage: { total_tokens: 150 } },
      ]
      result = Result.new(agent: "test", logs: logs)
    ...
    def test_agents_involved_with_single_agent
      logs = [
        { type: "llm_response", agent: "backend" },
    ...
    def test_agents_involved_with_multiple_agents
      logs = [
        { type: "llm_response", agent: "lead" },
        { type: "tool_call", agent: "backend" },
        { type: "llm_response", agent: "backend" },
        { type: "tool_call", agent: "frontend" },
        { type: "llm_response", agent: "frontend" },
        { type: "llm_response", agent: "backend" }, # Duplicate
      ]
      result = Result.new(agent: "test", logs: logs)
    ...
    def test_agents_involved_returns_symbols
      logs = [
        { type: "llm_response", agent: "backend" },
        { type: "llm_response", agent: :frontend }, # Already a symbol
      ]
      result = Result.new(agent: "test", logs: logs)
    ...
    def test_log_aggregation_methods_work_together
      logs = [
        { type: "llm_response", agent: "lead", usage: { total_cost: 0.01, total_tokens: 100 } },
        { type: "tool_call", agent: "backend" },
        { type: "llm_response", agent: "backend", usage: { total_cost: 0.02, total_tokens: 150 } },
        { type: "tool_call", agent: "frontend" },
        { type: "llm_response", agent: "frontend", usage: { total_cost: 0.015, total_tokens: 120 } },
      ]
      result = Result.new(agent: "test", logs: logs)

      assert_in_delta(0.045, result.total_cost)
      assert_equal(370, result.total_tokens)
